...
if File.exist?(first_arg)
  ...
  command = "/usr/bin/osascript #{script_arg} #{Shellwords.escape(path)} "\
    '>/dev/null'
  exec(command)
end
...
command = "/usr/bin/osascript #{script_arg} #{Shellwords.escape(first_arg)} "\
  "#{Shellwords.escape(dir)} >/dev/null"
